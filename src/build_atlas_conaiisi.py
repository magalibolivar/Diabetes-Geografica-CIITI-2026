# -*- coding: utf-8 -*-
"""
build_atlas_conaiisi.py
Genera el paper del Atlas Geoespacial de Diabetes en Argentina
en formato estricto CoNaIISI utilizando la plantilla oficial
paper/Determinantes_Geoespaciales_Diabetes_UAI_CONAIISI.docx.
"""
...
OUT_PATHS = [
    ROOT / "paper" / "Atlas_CONAIISI.docx",
    ROOT / "paper" / "Atlas_Diabetes_Argentina_CONAIISI.docx",
    ROOT / "paper" / "Determinantes_Geoespaciales_Diabetes_Argentina_CONAIISI.docx"
]

def set_section_columns(section, num_cols=2, space_twips=448):
    """Configura el numero de columnas en una seccion."""
...
def add_p(doc, text="", style="Normal", align=None, bold=None, italic=None, space_after=None, first_indent=None):
    """Agrega un parrafo asignando el estilo oficial de la plantilla."""
...
def add_runs_p(doc, segments, style="Normal", align=None, space_after=None, first_indent=None):
    """Agrega parrafo con multiples segmentos de texto (runs) con estilo."""
    p = doc.add_paragraph(style=style)
    if align is not None:
        p.alignment = align
    if space_after is not None:
...
def insert_fig(doc, img_path, caption, width_cm=16.0):
    """Inserta una figura centrada seguida de su epigrafe con estilo Caption."""
    p_img = doc.add_paragraph()
    p_img.alignment = AL.CENTER
    p_img.paragraph_format.space_before = Pt(4)
    p_img.paragraph_format.space_after = Pt(2)
    p_img.add_run().add_picture(str(img_path), width=Cm(width_cm))
    
    p_cap = doc.add_paragraph(caption, style="Caption")
    p_cap.alignment = AL.CENTER
    p_cap.paragraph_format.space_before = Pt(2)
    p_cap.paragraph_format.space_after = Pt(6)
    return p_img, p_cap

def insert_table(doc, csv_path, caption, col_widths=None):
    """Inserta una tabla con estilos oficiales Table Heading y Table Contents."""
    p_cap = doc.add_paragraph(caption, style="Caption")
    p_cap.alignment = AL.CENTER
...
def build_paper():
    print(f"Cargando plantilla oficial CoNaIISI desde:\n  {TEMPLATE_PATH}")
    doc = Document(TEMPLATE_PATH)
    
    # Vaciar elementos de cuerpo manteniendo estilos
    ...
    # ==========================================================================
    # SECCIÓN 0: PORTADA, RESUMEN Y ABSTRACT (1 COLUMNA)
    # ==========================================================================
    s0 = doc.sections[0]
    ...
    add_p(doc, "Atlas Geoespacial de la Diabetes en Argentina (CoNaIISI): Disparidad Territorial en la Brecha de Tratamiento e Integración Multifuente (ENFR 2018 e INDEC)",
          style="Título Principal", align=AL.CENTER, space_after=4)
    
    # Título en inglés
    add_p(doc, "Geospatial Atlas of Diabetes in Argentina (CoNaIISI): Territorial Disparities in Treatment Gap and Multi-Source Data Integration",
          style="Address", align=AL.CENTER, italic=True, space_after=10)
    
    # Autores
    add_p(doc, "María Florencia Rossi, Magali Bolivar, Matías Montiel, Roxana Martínez, Nestor Balich, Franco Balich",
          style="Address", align=AL.CENTER, bold=True, space_after=2)
          
    # Filiación institucional
    add_p(doc, "CAETI - Centro de Altos Estudios en Tecnología Informática\n"
          ...
          style="Address", align=AL.CENTER, space_after=3)
          
    # Correos
    add_p(doc, "{MariaFlorencia.Rossi, MagaliFlorencia.BolivarCruz, MatiasNicolas.MontielTorres}@alumnos.uai.edu.ar\n"
               "{Roxana.Martinez, nestor.balich, francoadrian.balich}@uai.edu.ar",
          style="Address", align=AL.CENTER, italic=True, space_after=10)
          
    # Resumen
    add_p(doc, "Resumen", style="Título 1er Nivel", space_after=2)
    add_p(doc, "La prevalencia de diabetes mellitus y el acceso efectivo a su tratamiento exhiben disparidades territoriales severas que desafían la equidad del sistema de salud. Cuantificar estas diferencias en la Argentina requiere integrar fuentes públicas heterogéneas que operan a distintas resoluciones y marcos metodológicos. Este trabajo presenta un pipeline reproducible de ciencia de datos que consolida los microdatos de la 4ª Encuesta Nacional de Factores de Riesgo (ENFR 2018), indicadores censales de cobertura y Necesidades Básicas Insatisfechas (Censo 2010/2022) y estadísticas de pobreza de la Encuesta Permanente de Hogares (EPH 2018) para las 24 jurisdicciones de la Argentina, contextualizándolas con series globales de la International Diabetes Federation (IDF) y NCD-RisC. El análisis revela una pronunciada brecha territorial: mientras la prevalencia provincial oscila entre 8,8% (CABA) y 17,3% (San Luis), la proporción de personas con diabetes bajo tratamiento farmacológico varía entre un mínimo crítico del 32,7% (La Pampa) y un 70,0% (Santa Cruz y Tierra del Fuego). Crucialmente, la brecha de tratamiento no correlaciona con la falta de cobertura formal de salud (r = 0,12; p = 0,58) ni con los niveles de pobreza monetaria (r = -0,19; p = 0,37), lo que demuestra que la cobertura de medicación depende de dinámicas locales de gestión sanitaria más que del nivel de ingreso. En el plano metodológico, se evalúan modelos predictivos supervisados (OLS y Random Forest), evidenciando que con N=24 unidades provinciales los modelos múltiples sufren de falta de potencia estadística (R² LOOCV ≤ 0), lo que fundamenta la conveniencia de privilegiar un atlas descriptivo y geoespacial frente a modelos predictivos sobreajustados. El trabajo aporta herramientas cartográficas y estadísticas de libre acceso para la planificación focalizada de políticas sanitarias en el territorio nacional.",
          style="Abstract", space_after=4)
          
    add_runs_p(doc, [
        ("Palabras clave: ", True, False),
    ...
    ], style="Abstract", space_after=8)
    
    # Abstract
    add_p(doc, "Abstract", style="Título 1er Nivel", space_after=2)
    ...
    ], style="Abstract", space_after=10)
    
    # ==========================================================================
    # SECCIÓN 1: CUERPO PRINCIPAL (2 COLUMNAS)
    # ==========================================================================
    ...
    add_p(doc, "2. Marco conceptual y antecedentes", style="Título 1er Nivel")
    ...
    # ==========================================================================
    # SECCIÓN 3: RESULTADOS 4.1 (2 COLUMNAS)
    # ==========================================================================
    ...
    set_section_columns(s3, num_cols=2, space_twips=448)
    
    add_p(doc, "4. Resultados", style="Título 1er Nivel")
    add_p(doc, "4.1. Atlas de prevalencia y factores de riesgo", style="Título 2do Nivel")
    add_p(doc, "La distribución geográfica de la prevalencia de diabetes exhibe un marcado patrón territorial (Figura 1a). La tasa promedio nacional de 12,7% enmascara brechas pronunciadas entre jurisdicciones: la Ciudad Autónoma de Buenos Aires registra el valor más bajo del país (8,8%), seguida por Jujuy (8,9%) y Chaco (10,3%). En el extremo opuesto, provincias de la región de Cuyo y el Centro muestran tasas significativamente superiores: San Luis encabeza la prevalencia nacional con 17,3%, seguida por San Juan (15,9%), Tierra del Fuego (15,9%), La Rioja (15,1%) y La Pampa (14,6%). Todas las estimaciones de la ENFR presentan coeficientes de variación muestral (CV) inferiores al 12,5%, confirmando su robustez estadística.", style="Normal")
    add_p(doc, "Los factores de riesgo cardiometabólicos asociados presentan una heterogeneidad espacial igualmente acentuada (Figura 2). La obesidad autorreportada (IMC≥30) abarca desde 17,0% en CABA hasta 34,4% en San Juan y 34,0% en Santa Cruz. La presión arterial elevada alcanza su pico en la región Noreste (NEA), liderada por Formosa con 52,2% y Corrientes con 40,2%, frente al 26,6% de CABA. Asimismo, el sedentarismo o inactividad física oscila entre 23,2% (San Juan) y 69,1% (Formosa).", style="Normal")
    add_p(doc, "El acceso al diagnóstico preventivo —medido a través de la realización de glucemia alguna vez en la vida— exhibe un gradiente socio-territorial persistente: mientras en CABA y Santa Fe supera el 89-92%, en provincias del NEA y NOA (Chaco 60,3%, Santiago del Estero 60,6%, Formosa 61,1%) cuatro de cada diez adultos nunca se han testeado el nivel de glucosa en sangre, sugiriendo la presencia de un importante subdiagnóstico en dichas regiones.", style="Normal")
    
    # ==========================================================================
    # SECCIÓN 4: TABLA 1 RESUMEN REGIONAL FULL WIDTH (1 COLUMNA)
    # ==========================================================================
    ...
    # ==========================================================================
    # SECCIÓN 5: FIGURA 2 FACTORES DE RIESGO FULL WIDTH (1 COLUMNA)
    # ==========================================================================
    ...
    # ==========================================================================
    # SECCIÓN 6: RESULTADOS 4.2 BRECHA DE TRATAMIENTO (2 COLUMNAS)
    # ==========================================================================
    ...
    set_section_columns(s6, num_cols=2, space_twips=448)
    
    add_p(doc, "4.2. Disparidad territorial en la brecha de tratamiento", style="Título 2do Nivel")
    add_p(doc, "El hallazgo empírico central del presente estudio es la extrema disparidad geográfica en la cobertura de tratamiento de la diabetes (Figura 1b y Figura 3). Entre la población que declaró tener diagnóstico de la enfermedad, el porcentaje que recibe tratamiento farmacológico regular (hipoglucemiantes orales o insulina) varía en más del doble entre jurisdicciones:", style="Normal")
    add_p(doc, "En el extremo inferior se sitúa La Pampa, donde solo el 32,7% de los diabéticos se encuentra bajo tratamiento activo (prácticamente siete de cada diez pacientes sin cobertura farmacológica), seguida por Chubut (38,3%), Corrientes (44,2%) y Río Negro (45,2%). En contraste, jurisdicciones de la Patagonia austral como Santa Cruz (70,0%) y Tierra del Fuego (70,0%), junto con La Rioja (66,2%), Santa Fe (65,9%) y Formosa (62,6%), alcanzan coberturas significativamente superiores.", style="Normal")
    add_p(doc, "La hipótesis convencional postularía que la brecha de tratamiento responde a barreras socioeconómicas individuales: menor cobertura en provincias más pobres o con mayor proporción de población sin seguro médico formal (dependiente exclusivamente del hospital público). Sin embargo, el análisis empírico refuta esta premisa (Figura 3b): la tasa de tratamiento provincial no correlaciona de manera estadísticamente significativa con el porcentaje de población sin cobertura médica privada u obra social (r = +0,12; p = 0,58), ni con la tasa de pobreza de personas de la EPH (r = -0,19; p = 0,37), ni con el NBI de hogares (r = +0,10; p = 0,65).", style="Normal")
    add_p(doc, "Provincias con elevada vulnerabilidad social y más del 50% de su población dependiente del sistema público de salud (como Formosa o Chaco) logran tasas de tratamiento superiores al 54-62%, mientras que distritos con menor pobreza estructural (como La Pampa o Chubut) exhiben las coberturas más bajas del país. Esto evidencia que la provisión continua y la adherencia al tratamiento farmacológico de la diabetes están fuertemente condicionadas por la organización del primer nivel de atención y las políticas de distribución de medicamentos provinciales (v.g. programas Remediar/Redes), más que por la capacidad económica individual de las familias.", style="Normal")
    
    # ==========================================================================
    # SECCIÓN 7: FIGURA 3 BRECHA DE TRATAMIENTO FULL WIDTH (1 COLUMNA)
    # ==========================================================================
    ...
    # ==========================================================================
    # SECCIÓN 8: RESULTADOS 4.3 CORRELACIONES (2 COLUMNAS)
    # ==========================================================================
    ...
    set_section_columns(s8, num_cols=2, space_twips=448)
    
    add_p(doc, "4.3. Matriz de correlaciones y diagnóstico de escala", style="Título 2do Nivel")
    add_p(doc, "La matriz de correlación bivariada (Figura 4) ilustra las interrelaciones entre los determinantes analizados. A diferencia de lo observado en escalas con gran número de unidades territoriales (como los 51 estados de EE.UU. o condados), a nivel provincial argentino las asociaciones directas con la prevalencia de diabetes son débiles o contraintuitivas.", style="Normal")
    add_p(doc, "La única covariable que muestra una correlación lineal moderada con la prevalencia de diabetes es la densidad poblacional logarítmica (r = -0,44; p = 0,032), sugiriendo que las provincias más densamente urbanizadas reportan menor prevalencia (CABA 8,8%) en comparación con las provincias de menor densidad del interior (San Luis 17,3%).", style="Normal")
    ...
    # ==========================================================================
    # SECCIÓN 9: FIGURAS 4 Y 5 + TABLA 2 MODELOS FULL WIDTH (1 COLUMNA)
    # ==========================================================================
    s9 = doc.add_section(WD_SECTION_START.CONTINUOUS)
    s9.top_margin, s9.bottom_margin = Cm(2.0), Cm(2.5)
    s9.left_margin, s9.right_margin = Cm(1.75), Cm(1.75)
    ...
    # ==========================================================================
    # SECCIÓN 10: RESULTADOS 4.4, 4.5, DISCUSIÓN, CONCLUSIONES Y REFS (2 COLUMNAS)
    # ==========================================================================
    ...
    for r in refs:
        add_p(doc, r, style="References", space_after=2.5)
